<template>
    <Row class="vm-table vm-panel">
        ...
        <div class="panel-body">
            ...
            <div class="edit" v-if="this.type === 1 || this.type === 4">
            ...
            <Row type="flex" justify="space-between" class="footer">
                <div class="info-bar">
                    Show<Input-number class="input-number" v-model="showNum" :max="tripData.length" :min="1" @on-change="updataShowData ">{{ showNum }}</Input-number>/ Page
                </div>
                <div class="page">
                <span class="total">Total {{ tripData.length }}</span>
                <Page :total="tripData.length" :current="currentPage" :page-size="showNum" @on-change="pageChange"></Page>
                </div>
            </Row>
        </div>
        ...
        <Modal
            ...
            <Form :label-width="80">
                ...
                <Form-item label="出差类型">                
                    <RadioGroup v-model="dataEdit['type']" type="button" label="出差类型">
                        <Radio label="1">公司指派</Radio>
                        <Radio label="2">个人申请</Radio>
                    </RadioGroup>
                </Form-item>
                <Form-item label="出差理由">
                    <Input v-model="dataEdit['reason']" type="textarea" placeholder="请输入出差理由" label="出差理由"></Input>
                </Form-item>
        ...
        <Modal
            ...
            <Form :label-width="80">
                <Form-item label="员工工号" v-if="this.type === 4">
                    <Input v-model="dataAdd['user_id']" placeholder="请输入工号" label="出差理由"></Input>                
                </Form-item>
                <Form-item label="开始日期">
                ...
                <Form-item label="出差类型">
                    <RadioGroup v-model="dataAdd['type']" type="button" label="出差类型">
                        <Radio label="1">公司指派</Radio>
                        <Radio label="2">个人申请</Radio>
                    </RadioGroup>
                </Form-item>
                <Form-item label="出差理由">
                    <Input v-model="dataAdd['reason']" type="textarea" placeholder="请输入出差理由" label="出差理由"></Input>                         
                </Form-item>
                <Form-item label="申请状态"  v-if="this.type === 4">
                    <RadioGroup v-model="dataEdit['status']" type="button" label="申请状态">
                        <Radio label="1">已保存</Radio>
...
export default {
    ...
    methods: {
        ...
        replyOk: function (data) {
            ...
            this.$socket.emit('', obj, function (status) {
                if (status) {
                    this.tripData.forEach(function (elem) {
                        if (elem['number'] === data['number']) {
                            elem['status'] = 4;
        ...
        deleteOk: function (data) {
            ...
            for (let item in data) {
                ...
                this.$socket.emit('', obj, function (status) {
                    if (status) {
                        this.tripData = this.tripData.filter(i => i['number'] !== item['number']);
                    } else {
        ...
        updataShowData: function () {
            ...
            this.showData = this.tripData.slice(startPage, endPage);
        },
        search: function () {
            let that = this;
            let tempData = that.tripData;
            that.showData = [];
    ...
    computed: {
        showColumns: function () {
            let showColumn = this.tripColumns.slice();
    ...
    watch: {
        tripData: function () {
            this.showData = this.tripData.slice(0, this.showNum);
        },
        dataDelete: function () {
